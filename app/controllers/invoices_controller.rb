class InvoicesController < ApplicationController
  ...
  def create
    ...
    if service.rate_limited?
      ...
      # Store the (large) parse result server-side, keyed by a short token in the
      # session cookie. Storing it directly in the cookie blew past the 4KB limit
      # (ActionDispatch::Cookies::CookieOverflow) on rich receipts.
      session[:invoice_token] = SecureRandom.uuid
      write_invoice(:result, result)
      redirect_to invoice_path(id: "current")
    end
  end

  def show
    result = read_invoice(:result)
    if result.present?
  ...
  def verify
    result = read_invoice(:result)&.deep_symbolize_keys
    ...
    if verification[:success]
      write_invoice(:cookies, filler.cookies)
    end
  ...
  INVOICE_CACHE_TTL = 1.hour

  # Large invoice state (parse result + platform cookies) lives in Rails.cache,
  # keyed by a short token held in the session cookie, so it never bloats the
  # cookie. Small scalars (RFC, file paths, flags) stay in the session directly.
  def invoice_cache_key(field)
    "invoices:#{session[:invoice_token]}:#{field}"
  end

  def read_invoice(field)
    return nil if session[:invoice_token].blank?
    Rails.cache.read(invoice_cache_key(field))
  end

  def write_invoice(field, value)
    return if session[:invoice_token].blank?
    Rails.cache.write(invoice_cache_key(field), value, expires_in: INVOICE_CACHE_TTL)
  end
